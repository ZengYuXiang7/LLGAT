...
def calculate_flops_params(model, sample_input, args):
    ...
    flops, params = profile(model, inputs=sample_input[:-1], verbose=False)
    # args.log.only_print(f"Flops: {flops} Params: {params}")
    return flops, params
...
def calculate_inference_time(model, sample_input, args):
    ...
    # args.log.only_print(f"Average Inference Time: {inference_time * 1000:.2f} ms")
